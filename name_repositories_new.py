    ...
    def CheckOut(self, org_name):
        ...
        for repos in self.org.get_repos():
            ...
            for stat in user_poll:
                count += 1
                my_login = stat.author.login
                worksheet.write('A' + str(count), my_login)
                if my_login == owner_login:
                    worksheet.write('B' + str(count), 'owner')
                else:
                    worksheet.write('B' + str(count), 'collaborator')
                NameUser = self.g.get_user(my_login)
                my_email = NameUser.email
                worksheet.write('C' + str(count), my_email)
...
if __name__ == '__main__':
    RepositoryCreation().CheckOut('openstack')
